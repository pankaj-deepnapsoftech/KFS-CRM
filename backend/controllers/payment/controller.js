...
const getAllPayments = TryCatch(async (req, res) => {
  ...
  if (req.user.role === "Super Admin") {
    payments = await paymentModel.find().populate({
      path: "invoice",
    ...
    payments = await paymentModel.find({creator: req.user.id}).populate({
      path: "invoice",
      select: "invoicename customer",
      populate: {
        path: "customer",
...
const downloadPayment = TryCatch(async (req, res) => {
  ...
  pdf.fontSize(14);
  pdf.text(
    "KFS Fitness, Kuber Tower, Sector-20B, Opp: Thomson Press, Ajronda, Faridabad (HARYANA) - 121007",
    { width: 300 }
  );
